        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (!mScroller.isFinished()) {
                    ...
                    // TODO: 可以结合scrollTo，先停止上面Scroller的动画，然后立马到达最终的位置
                    // scrollTo(mScroller.getFinalX(),mScroller.getFinalY());
                }
                mScrollStart = getScrollY();
                Log.i(TAG, "ACTION_DOWN:   y=" + y + " , getScrollY=" + getScrollY());
                mLastY = y;
                break;
            case MotionEvent.ACTION_MOVE:
                ...
                Log.w(TAG, "ACTION_MOVE:   y=" + y + " , getScrollY=" + getScrollY());
                /**
                 * 边界条件1：已经到达顶端，下拉多少，最终的值都是偏移0
                 * TODO: 1.dy<0表示的是往下移动
                 *       2.当在第0页的时候，再往下移动，scrollY始终是0
                 *       3.如果不这样做的话，任后面的scrollBy（dy）的话，就是会露出来容器颜色
                 */
                ...
                /**
                 * 边界条件2：已经到达底部，上拉多少，就往下滚动多少【最终目的仍然是达到底部不然继续滑动了】
                 * 采取的方式是利用scrollerBy滚动回来
                 */
                ...
                // TODO: scrollBy是移动偏移，想象初始位置一直在变化，对应的是mLastY是一直在重新赋值的
                // TODO: dy的正值表示往正方向移动
                scrollBy(0, dy);
                mLastY = y;
                break;
            case MotionEvent.ACTION_UP:
                ...
                if (wantScrollToPre())// 往下滑动
                {
                    if (shouldScrollToPre()) {
                        // TODO: 在getScrollY值的基础上再偏移dy,此时计算的dy为负数，往下移动剩余屏距离
                        mScroller.startScroll(0, getScrollY(), 0, -mScreenHeight - dScrollY);
