...
public class GestureLockView extends View {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        // TODO: 很细节,计算半径的时候还考虑到画笔边框的宽度
        mRadius -= mStrokeWidth / 2;

        // 绘制三角形，初始时是个默认箭头朝上的一个等腰三角形，用户绘制结束后，根据由两个GestureLockView决定需要旋转多少度
        float mArrowLength = mWidth / 2f * mArrowRate;
        mArrowPath.moveTo(mWidth / 2f, mStrokeWidth + 2);
        mArrowPath.lineTo(mWidth / 2f - mArrowLength, mStrokeWidth + 2
                + mArrowLength);
        mArrowPath.lineTo(mWidth / 2f + mArrowLength, mStrokeWidth + 2
                + mArrowLength);
        mArrowPath.close();
    ...
    private void drawArrow(Canvas canvas) {
        if (mArrowDegree != -1) {
            ...
            // TODO: 很巧妙,旋转画板,画完然后复原[相当于还是以原来的标准画三角形,只是最后画出的结果是画板旋转后的,妙呀~]
            //  往右旋转为正方向 , 可参考: https://blog.csdn.net/iasxk/article/details/17411381
            canvas.save();
